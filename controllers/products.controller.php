			   ...
			   preg_match('/^[0-9.]+$/', $_POST["editSellingPrice"])){
				...
				if($answer == "ok"){
					...
					echo'<script>
						...
						swal({
							  type: "success",
							  title: "El consumo ha sido modificado",
							  showConfirmButton: true,
							  confirmButtonText: "Close"
							  }).then(function(result){
										if (result.value) {
				...
				echo'<script>
					...
					swal({
						  type: "error",
						  title: "¡El consumo no puede tener campos en blanco o carácteres especiales!",
						  showConfirmButton: true,
						  confirmButtonText: "Close"
						  }).then(function(result){
							if (result.value) {
	...
	static public function ctrDeleteProduct(){
		...
		if(isset($_GET["idProduct"])){
			...
			if($answer == "ok"){
				...
				swal({
					  type: "success",
					  title: "El consumo ha sido eliminado satisfactoriamente",
					  showConfirmButton: true,
					  confirmButtonText: "Close"
					  }).then(function(result){
								if (result.value) {
